...
namespace DB
{
  ...
  {
  public:
    ErrorPrinter(ErrorFormat format, DiskSourceTree *tree = NULL) : found_errors_(false) {
      printf("%u", format);
    }
  ...
  private:
    void AddErrorOrWarning(const string & filename, int line, int column, const string & message, const string & type,
                           ostream & out)
    {
  ...
  void ProtobufRowInputStream::validateSchema()
  {
	...
	if (protobuf_type_to_column_type_name.find(protobuf_type) == protobuf_type_to_column_type_name.end())
	  {
	    throw Exception("Unsupported type " + string(field->type_name()) + " of a column " + header_column.name/*, ErrorCodes::TODO*/);
	  }
  ...
  ProtobufRowInputStream::ProtobufRowInputStream(
						 ...
						 const String & schema_dir, const String & root_type)
    : istr(istr_), header(header_), log(&Poco::Logger::get("ProtobufRowInputStream"))
  {
