...
/** Merge-selection predicate for CloudMergeTree, modeled on MergeTreeMergePredicate but trimmed:
  * no mutations (no mutation-version check -- CloudMergeTree has none yet), no patch parts
  * (getPatchesToApplyOnMerge always returns {}), no MVCC transactions (every part is stamped
  * Tx::NonTransactionalTID; see StorageCloudMergeTree::updatePartSetFromKeeper(), so there is no
  * snapshot/visibility concept to check here).
  *
  * What's left -- partition equality, the avoid-merges-volume and projection-set checks, and the
  * outdated-part-level gap check via getMaxLevelInBetween() -- is unchanged from
  * MergeTreeMergePredicate: none of it is specific to how a part got into the set (Keeper-synced
  * vs. a local disk listing), only to what's currently in it.
  */
